...
- Every zettel has a uid generated by org's own `org-id-new`, so the format is controlled by `org-id-method`.
- Filenames follow `org-zk-filename-style`: by default `slug_uid.org` (a lowercased, underscore-separated title), or `title (uid).org` when set to `title`.
- The same uid is stored in the file's `:id:` property and registered with `org-id` at creation time, so native `id:` links resolve immediately.
- Tags live on a `#+filetags:` line in the `:tag1:tag2:` format.
- Links can be inserted bare `[[id:uid]]` or with the target's title as the description `[[id:uid][title]]`; each form has its own command, for both new and existing zettels.
